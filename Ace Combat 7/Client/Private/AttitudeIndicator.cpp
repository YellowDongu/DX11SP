...
HRESULT AttitudeIndicator::Start(void)
{
	...
	CreateScale(part.texture, scale);
	...
	CreateScale(part.texture, scale); scale *= 1.2f;
	innerErrorCheck(CreateVertex(part.vertexBuffer, part.indexBuffer, part.indexCount, scale), L"Vertex/Index Create");
...
void AttitudeIndicator::Render(void)
{
	...
	float indicatorDistanceCalculated = indicatorDistance * (1.0f + (1.0f - std::abs(std::abs(transform->Angle().z) - 90.0f) / 90.0f * 0.9f));
	...
	for (INT i = 0; i < 5; i++)
	{
		...
		//position = rotatePoint({ 0.0f, indicatorDistance * static_cast<FLOAT>(i - 2) + (indicatorDistance * PitchAdjustValue) }, transform->Angle().z, windowSizeX, windowSizeY);
