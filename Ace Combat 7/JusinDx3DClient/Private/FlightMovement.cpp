...
void FlightMovement::CalculateMovement(void)
{
	...
	float newVelocity = (EnginePower * throttle - velocity) - velocity * speedFactor;
	...
	//std::cout << velocity << "	: " << newVelocity << "	: " << gravityFactor << std::endl;
...
void FlightMovement::CalculatePrimarySurfaceForce(void)
{
	if(yoke.x == 0.0f && manuverForce.x != 0.0f)
		manuverForce.x += manuverForce.x * -0.1f/*deltatime * 10.0f*/;
	else
	{
	...
	if (yoke.y == 0.0f && manuverForce.y != 0.0f)
		manuverForce.y += manuverForce.y * -0.1f/*deltatime * 10.0f*/;
	else
	{
	...
	//std::cout << manuverForce.x << "	: " << manuverForce.y << "	: " << manuverForce.z << std::endl;
...
void FlightMovement::Update(void)
{
	...
	transform->Translate(force * 0.0001f/*deltatime*/);
	//transform->Translate(force * DeltaTime());
}
