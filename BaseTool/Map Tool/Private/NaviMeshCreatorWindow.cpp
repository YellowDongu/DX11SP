...
void NaviMeshCreatorWindow::Start(void)
{
	...
	dsDesc.DepthEnable = FALSE;  // 깊이 테스트 끔
...
void NaviMeshCreatorWindow::Update(void)
{
	...
	if (VertexSetMode)
	{
		if (::Input()->getMouseButtonDown(MouseKey::LButton))
		{
			...
			DirectX::XMVECTOR rayView = DirectX::XMVector3TransformCoord(DirectX::XMVectorSet(((2.0f * ::Input()->getMousePosition().x) / viewPort->Width - 1.0f), ((-2.0f * ::Input()->getMousePosition().y) / viewPort->Height + 1.0f), 1.0f, 1.0f), DirectX::XMMatrixInverse(nullptr, DirectX::XMLoadFloat4x4(&::EngineInstance()->ProjectionMatrix())));

			rayOrigin = DirectX::XMVectorSet(0, 0, 0, 1);
			rayDir = DirectX::XMVector3Normalize(rayView);

			DirectX::XMMATRIX invView = DirectX::XMMatrixInverse(nullptr, DirectX::XMLoadFloat4x4(&::EngineInstance()->ViewMatrix()));
