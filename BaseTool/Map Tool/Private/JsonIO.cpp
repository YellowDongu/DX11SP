...
HRESULT CJsonIO::Save(ObjectManager* objectManager)
{
	if (objectManager == nullptr)
		return E_FAIL;

	std::wstring saveFilePath = selectSaveFilePath();
	if (lstrlenW(saveFilePath.data()) == 0)
	{
		Console::WriteLine("Cancel save");
		return E_FAIL;
	}

	Console::WriteLine(std::string("saving at") + To_String(saveFilePath));

	try
	{
		nlohmann::json jsonObject;
		if (FAILED(SaveGameObjects(&objectManager->GameObjects(), jsonObject)))
			return E_FAIL;
...
HRESULT CJsonIO::SaveScene(ObjectManager* objectManager, std::wstring saveFilePath)
{
	if (objectManager == nullptr)
		return E_FAIL;
...
HRESULT CJsonIO::SaveGameObjects(std::list<GameObjectMetaData*>* targetList, nlohmann::json& JSONObject)
{
	std::vector<nlohmann::json> gameObjects;
	...
	try
	{
		...
		{
			nlohmann::json gameObjectJSON;
			SaveGameObjectMetaData(gameObject, gameObjectJSON);
	...
	{
		Console::WriteLine(std::string("Error::serialize to JSON : ") + ex.what());
		return E_FAIL;
	}
	JSONObject["GameObjectList"] = gameObjects;
...
HRESULT CJsonIO::SaveGameObjectMetaData(GameObjectMetaData* object, nlohmann::json& JSONObject)
{
	SavedGameObjectMetaData data;
	object->CreateSaveData(data);

	JSONObject["name"] = data.name;
	JSONObject["layerName"] = data.layerName;
	JSONObject["presetName"] = data.presetName;
	JSONObject["renderType"] = static_cast<INT>(data.renderType);

	savefloat3(data.postion, "position", JSONObject);
	savefloat4(data.rotation, "rotation", JSONObject);
	savefloat3(data.scale, "scale", JSONObject);

	return S_OK;
}


HRESULT CJsonIO::savefloat3(const Vector3& vector, std::string variableName, nlohmann::json& JSONObject)
{
	nlohmann::json float3JSON;
	float3JSON["x"] = vector.x;
	float3JSON["y"] = vector.y;
	float3JSON["z"] = vector.z;
	JSONObject[variableName] = float3JSON;
	return S_OK;
}

HRESULT CJsonIO::savefloat4(const float4& vector, std::string variableName, nlohmann::json& JSONObject)
{
	nlohmann::json float4JSON;
	float4JSON["x"] = vector.x;
	float4JSON["y"] = vector.y;
	float4JSON["z"] = vector.z;
	float4JSON["w"] = vector.w;
	JSONObject[variableName] = float4JSON;
	return S_OK;
}

HRESULT CJsonIO::Loadfloat3(Vector3& vector, std::string variableName, nlohmann::json& object)
{
	auto iterator = object.find(variableName);
	if (iterator == object.end())
		return E_FAIL;
	nlohmann::json& JSONObject = *iterator;

	auto xIterator = JSONObject.find("x");
	if (xIterator == JSONObject.end())
		return E_FAIL;
	auto yIterator = JSONObject.find("y");
	if (yIterator == JSONObject.end())
		return E_FAIL;
	auto zIterator = JSONObject.find("z");
	if (zIterator == JSONObject.end())
		return E_FAIL;

	vector.x = static_cast<FLOAT>(*xIterator);
	vector.y = static_cast<FLOAT>(*yIterator);
	vector.z = static_cast<FLOAT>(*zIterator);

	return S_OK;
}

HRESULT CJsonIO::Loadfloat4(float4& vector, std::string variableName, nlohmann::json& object)
{
	auto iterator = object.find(variableName);
	if (iterator == object.end())
		return E_FAIL;
	...
	if (zIterator == JSONObject.end())
		return E_FAIL;
	auto wIterator = JSONObject.find("w");
	if (wIterator == JSONObject.end())
		return E_FAIL;

	vector.x = static_cast<FLOAT>(*xIterator);
	vector.y = static_cast<FLOAT>(*yIterator);
	vector.z = static_cast<FLOAT>(*zIterator);
	vector.w = static_cast<FLOAT>(*wIterator);

	return S_OK;
}

void CJsonIO::LoadJsonString(std::string& target, std::string variableName, nlohmann::json& JSONObject)
{
	if (JSONObject[variableName].is_string())
	{
		const std::string& source = JSONObject[variableName];
		target.resize(source.size());

		std::memcpy(&target[0], source.data(), source.size());
	}
}

HRESULT CJsonIO::Load(ObjectManager* objectManager)
{
	if (objectManager == nullptr)
		return E_FAIL;

	std::wstring filePath = selectLoadFilePath();

	if (lstrlenW(filePath.data()) == 0)
	{
		Console::WriteLine("Cancel load");
		return E_FAIL;
	}
	...
	try
	{
		for (auto& data : objectManager->GameObjects())
		{
			data->Free();
			delete data;
		}
		...
		LoadGameObjects(objectManager, objectManager->GameObjects(), loadedData);
...
HRESULT CJsonIO::Load_Scene(ObjectManager* objectManager, std::wstring saveFilePath)
{
	if (objectManager == nullptr)
		return E_FAIL;
...
HRESULT CJsonIO::LoadGameObjects(ObjectManager* objectManager, std::list<GameObjectMetaData*>& targetList, nlohmann::json& JSONObject)
{
	std::vector<nlohmann::json> gameObjects = JSONObject["GameObjectList"];

	for (auto& object : gameObjects)
	{
		if (FAILED(LoadGameObject(objectManager, objectManager->GameObjects(), object)))
			return E_FAIL;
	}
...
HRESULT CJsonIO::LoadGameObject(ObjectManager* objectManager, std::list<GameObjectMetaData*>& targetList, nlohmann::json& JSONObject)
{
	SavedGameObjectMetaData data;

	LoadJsonString(data.name, "name", JSONObject);
	LoadJsonString(data.layerName, "layerName", JSONObject);
	LoadJsonString(data.presetName, "presetName", JSONObject);
	data.renderType = static_cast<RenderType>(JSONObject["renderType"]);

	if (FAILED(Loadfloat3(data.postion, "position", JSONObject)))
		return E_FAIL;
	if (FAILED(Loadfloat4(data.rotation, "rotation", JSONObject)))
		return E_FAIL;
	if (FAILED(Loadfloat3(data.scale, "scale", JSONObject)))
		return E_FAIL;

	GameObjectMetaData* newObject = objectManager->GetGameObject(::To_WString(data.presetName));
	if (newObject == nullptr)
		return E_FAIL;
	newObject->LoadSaveData(data);

	targetList.push_back(newObject);
	return S_OK;
}
