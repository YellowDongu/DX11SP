...
void HierarchyWindow::RenderDraggableList(std::list<GameObjectMetaData*>& items)
{
    ...
    for (auto iterator = items.begin(); iterator != items.end(); ++iterator, ++index)
    {
        ...
        for (auto& selectedItem : *selectedData)
        {
        ...
        container = item->nameA + hider + std::to_string(index);
        ...
        if (ImGui::Selectable(container.c_str(), selected))
        {
            if (!selected)
            {
                ...
                objectManager->Select(*iterator);
            }
            else
            {
                objectManager->Unselect(*iterator);
            }
        }
...
void HierarchyWindow::LinkData(ObjectManager* target)
{
...
void HierarchyWindow::Free(void)
{
}

HierarchyWindow* HierarchyWindow::Create(ObjectManager* objectManager)
{
    if (objectManager == nullptr)
        return nullptr;
    HierarchyWindow* newInstnace = new HierarchyWindow();
    newInstnace->LinkData(objectManager);
    return newInstnace;
}

HRESULT HierarchyWindow::CreateFrame(void)
{
    ...
    if (ImGui::BeginPopupContextWindow())
    {
        if (!selectedData->empty())
        {
            ...
            if (ImGui::MenuItem("Remove"))
            {
                ...
                {
                    objectManager->Delete(selectedData->front());
                }
                objectManager->Unselect();
            }
            if (ImGui::MenuItem("Clone"))
            {
                GameObjectMetaData* metaData = nullptr;
                for (auto& selected : *selectedData)
                {
                    objectManager->Add(selected->Clone());
                }

            }

        }
        if (ImGui::BeginMenu("CreatePreset"))
        {
            for (auto& item : objectManager->LoadedGameObjects())
            {
                if (ImGui::MenuItem(item.second->nameA.c_str()))
                {
                    GameObjectMetaData* newInstance = item.second->Clone();
                    if(newInstance->gameObject != nullptr)
                        objectManager->Add(newInstance);
                    else
                        delete newInstance;
    ...
    if (rename)
    {
        ...
        {
            selectedData->front()->nameA = renameBuffer;
            selectedData->front()->name = To_WString(renameBuffer);
            renameBuffer = "";
            rename = false;
        }
...
void HierarchyWindow::AddGameObjectMenu(void)
{
...
void HierarchyWindow::CloneGameObject(GameObjectMetaData* metaData, GameObjectMetaData* target)
{
}

void HierarchyWindow::ShortcutKey(void)
{
